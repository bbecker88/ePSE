<%@ Page Title="" Language="C#" MasterPageFile="~/Views/Shared/ExposeSite.Master" Inherits="System.Web.Mvc.ViewPage<RubricOn.ViewModel.EvaluarRubricaExposeViewModel>" %>
...
<asp:Content ID="Content2" ContentPlaceHolderID="MainContent" runat="server">
    ...
    <% if(Model.RutaCancelado!="") {%>
        <%= Html.Link("Cancelar y regresar", Model.RutaCancelado, null)%>
    <%}%>
                   ...
                   var tdCriterioClass = EsAspectoPar ? " tdCriterioPar " : " tdCriterioImpar ";
                    ...
                    <tr>
                        ...
                        <% var checkBoxName = "EvalA" + aspecto.AspectoRubricaId + "-C" + criterio.CriterioRubricaId;
                           var isChecked = Model.RespuestasPlantilla.Any(x=>x.CriterioRubricaId == criterio.CriterioRubricaId);  %>
                        <td style=" <%= tdWidthStyle%>" class="<%= tdCriterioClass %>">
                            <% if(isChecked){ %>
                                <%= Html.CheckBox(checkBoxName,true, new { id = checkBoxName})%>
                            <%}else{ %>
                                <%= Html.CheckBox(checkBoxName,false, new { id = checkBoxName})%>
                            <%} %>
